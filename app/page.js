    ...
    <div className="auth-shell">
      ...
      <section className="auth-form-panel">
        <form className="auth-card" onSubmit={submit}>
          <div><span className="eyebrow">LINGUAFLOW V0.3</span><h2>{mode === "login" ? "Đăng nhập" : "Tạo tài khoản"}</h2><p>{mode === "login" ? "Tiếp tục quá trình học của bạn." : "Tạo tài khoản miễn phí để bắt đầu."}</p></div>
...
function ContentLab({ language, user, onUploaded }) {
  ...
  return <>
    <div className="page-title"><span className="eyebrow">LEARN FROM ANYTHING</span><h2>Học từ video & âm thanh</h2><p>Upload media của bạn. Bản V0.3 đã lưu file thật; AI transcript sẽ nối ở bước kế tiếp.</p></div>
    ...
    <div className="feature-grid">{[["01","Lưu media thật","Supabase Storage + thư viện riêng"],["02","Click để học","Tra nghĩa và lưu từ"],["03","Player","Phát lại audio/video đã upload"],["04","Sẵn cho AI","Transcript sẽ nối ở V0.4"]].map((x) => <div className="feature-card" key={x[0]}><span>{x[0]}</span><h4>{x[1]}</h4><p>{x[2]}</p></div>)}</div>
...
function Library({ user, refreshKey }) {
  ...
  return <>
    ...
    {loading ? <div className="empty-state"><div>◌</div><h3>Đang tải thư viện...</h3></div> : tab === "media" ? <>
      {selected && <section className="media-viewer"><div className="media-viewer-head"><div><span className="eyebrow">NOW LEARNING</span><h3>{selected.title}</h3></div><button className="text-btn" onClick={() => setSelected(null)}>Đóng</button></div>{selected.media_type === "video" ? <video controls src={selected.signedUrl} /> : <audio controls src={selected.signedUrl} />}<div className="transcript-empty"><b>Transcript chưa được tạo</b><p>V0.4 sẽ nối AI để tạo text + timestamp + Pinyin/phiên âm.</p></div></section>}
...
function Settings({ onClose }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><div className="modal" onMouseDown={(e) => e.stopPropagation()}><div className="modal-head"><div><span className="eyebrow">SETTINGS</span><h2>AI Provider</h2></div><button onClick={onClose}>×</button></div><p className="modal-copy">Bản đầu ưu tiên BYOK để giảm chi phí. Chúng ta sẽ nối lưu key an toàn khi làm V0.4.</p><div className="segmented"><button className={mode === "app" ? "selected" : ""} onClick={() => setMode("app")}>App AI</button><button className={mode === "byok" ? "selected" : ""} onClick={() => setMode("byok")}>My API Key</button></div>{mode === "byok" ? <div className="form-stack"><label>Provider<select><option>Gemini</option><option>OpenAI</option><option>Custom OpenAI-compatible</option></select></label><label>API Key<input type="password" placeholder="••••••••••••••••" /></label><label>Model<select><option>Auto</option></select></label><button className="primary-btn">Test connection</button></div> : <div className="notice">AI tích hợp sẵn sẽ bật khi có quota/subscription.</div>}</div></div>;
}
...
export default function Page() {
  ...
  return <div className="app-shell">
    ...
    <nav className="mobile-nav">{NAV.slice(0, 4).map(([id, icon, label]) => <button key={id} className={active === id ? "active" : ""} onClick={() => setActive(id)}><span>{icon}</span><small>{label.replace("Học ", "")}</small></button>)}</nav>{settings && <Settings onClose={() => setSettings(false)} />}
  </div>;
}
